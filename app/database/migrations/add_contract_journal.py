#!/usr/bin/env python3
"""
Migration script for creating contract_journal table.
...
def migrate_add_contract_journal() -> bool:
    db_path = get_db_path()
    ...
    try:
        ...
        if table_exists(cursor, "contract_journal"):
            print("✅ Table 'contract_journal' already exists. No migration needed.")
            conn.close()
            return True
        ...
        conn.commit()
        conn.close()
        print("✅ Created table 'contract_journal'")
        return True
...
if __name__ == "__main__":
    ok = migrate_add_contract_journal()
